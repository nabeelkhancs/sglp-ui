...
import { register } from "@/api/communications";
...
const SignupContainer = () => {
  ...
  const handlePersonalDetails = (data: { name: string; cnic: string; email: string }) => {
    setForm(prev => ({ ...prev, ...data }));
  };
  const handleEmployerDetails = (data: { govtID: string; designation: string; deptID: string; dptIdDoc: string }) => {
    setForm(prev => ({ ...prev, ...data }));
  };
  ...
  const handleNext = () => {
    if (step === 1) {
      const { valid, errors } = Validations.validatePersonalDetails(form.name, form.cnic, form.email);
      setErrors(errors);
      if (!valid) return;
      setStep(2);
    } else if (step === 2) {
      const { valid, errors } = Validations.validateEmployerDetails(form.govtID, form.designation, form.deptID, form.dptIdDoc);
      setErrors(errors);
      if (!valid) return;
      setStep(3);
    }
  ...
  const handleSubmit = async () => {
    const { valid, errors } = Validations.validatePassword(form.password);
    setErrors(errors);
    if (!valid) return;
    setLoading(true);
    try {
      ...
      <div>
        ...
        <div className="form-area">
          ...
          >
            ...
            {step === 3 && (
              <PasswordScreen
                values={{ password: form.password, confirmPassword: "" }}
